...
3. 用户本次指定的需求章节内容（从 Word 文件或 Markdown PRD 中提取）
...
- [ ] 已提取本次指定章节的需求内容（Word 或 Markdown）
...
| 输入来源 | 读取位置 | 重点提取 |
|---|---|---|
| Word 文档（推荐） | `inputs/requirements/raw_docs/<文件名>.docx` | 运行转换命令提取章节内容 |
...
- 同一条用例参考多个来源时，用顿号分隔，例如 `来源：需求文档、UI设计图、data_analysis_flow.md`。
- `testcase_templates/` 下的参考用例文件（如 `audit_trail_permission_manage.md`）只作为写法风格参考，不是需求来源，**不得**写入备注。
