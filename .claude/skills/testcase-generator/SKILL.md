...
1. 本 SKILL.md 文件
2. `generation_rules/quick_rules.md`
...
- 生成前检查 `inputs/ui_design/<章节名>/` 是否存在且含有非 README 文件
- 存在实际图片时读取，不存在时跳过，不影响生成流程
- 若章节名与目录名不完全匹配，尝试模糊匹配（如"报告编制"匹配"报告编制"目录）
...
| **另存** | 保留现有文件，将本次用例保存为带时间戳的新文件：`<module_name>_testcases_YYYYMMDD_HHMMSS.md` | 使用标准表头另存 |
...
| 内容 | 保存位置 |
|---|---|
| 原始测试用例（Markdown） | `outputs/origin_exports/<site_type>/<module_name>_testcases_YYYYMMDD_HHMMSS.md` |
| Excel 导出文件 | `outputs/excel_exports/<site_type>/<module_name>_testcases_YYYYMMDD_HHMMSS.xlsx` |
