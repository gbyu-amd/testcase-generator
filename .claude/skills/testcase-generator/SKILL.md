...
description: 根据淘宝项目的需求文档、界面设计图、接口文档、业务规则、历史缺陷和已有参考用例，生成测试用例并导出为 Excel。适用于登录、搜索、商品详情、购物车、结算下单、多页签操作等淘宝前端测试场景。
---
...
为淘宝类电商前端项目生成结构化功能测试用例。基于需求文档、界面设计图、接口文档、业务说明、历史缺陷或已有用例，生成可执行、可导出的测试用例。
...
7. `knowledge_base/historical_bugs/` 下的相关缺陷
8. `generation_rules/` 下的规则文件
9. `testcase_templates/common_templates/` 下的模板
10. `testcase_templates/modules/<module_name>/testcases.md`（作为参考，只读）
11. 用户本次提供的需求、界面设计图、接口文档或补充说明
...
1. 判断需求影响的功能模块
2. 提取页面入口、用户角色、前置条件、主流程、分支流程、字段规则、按钮状态、异常提示、数据依赖和历史风险
3. 设计正向流程、异常流程、边界值、字段校验、权限控制、状态变化、历史缺陷回归和跨模块联动用例
4. 参考 `testcase_templates/` 中已有用例，避免重复场景，但不要修改参考用例
5. 生成本次输出用例，编号从 001 开始递增
6. 保存到 `outputs/origin_exports/<module_name>_testcases.md`
7. 运行校验脚本：`python scripts/validate_cases.py --source outputs/origin_exports/<module_name>_testcases.md`
8. 如果校验失败，必须根据问题明细修复 Markdown 用例文件，然后重新运行校验
9. 校验通过后运行导出脚本：`python scripts/export_testcases.py --source outputs/origin_exports/<module_name>_testcases.md`
10. 回复导出文件路径、用例数量和覆盖模块
...
| 历史缺陷 | `knowledge_base/historical_bugs/` 或用户输入 | 缺陷触发条件、回归目标 |
