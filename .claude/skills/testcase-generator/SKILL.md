...
3. 用户本次指定的需求章节内容（从 `inputs/requirements/current_prd.md` 中提取；若用户指定 Word 文件，必须先转换覆盖该文件）
4. 对应章节的 UI 设计图目录 `inputs/ui_design/<章节名>/`（目录存在且有图片时读取）
5. `testcase_templates/modules/menu_index.md` 和命中的参考文件（按需读取）
6. `knowledge_base/project_overview.md`、`business_glossary.md`、`user_roles.md`（信息不足时读取）
7. `knowledge_base/core_flows/` 下的相关流程（按需读取）
8. `testcase_templates/common_templates/` 下的通用模板（按用例类型选用）

**UI 设计图读取规则：**

UI 设计图按章节名（中文）组织在 `inputs/ui_design/` 下，与提示词中的章节名直接对应：

- 生成前**必须**用命令行列出 `inputs/ui_design/` 下所有子目录，再与章节名匹配，不得用 Glob 模式匹配（中文目录名可能匹配失败）：

```bash
ls inputs/ui_design/
```

- Windows PowerShell 下可使用等价命令：

```powershell
Get-ChildItem inputs/ui_design/
```

- 目录名与章节名一致时读取其中所有图片；存在实际图片时读取，不存在时跳过，不影响生成流程
- 若目录名与章节名不完全一致，尝试模糊匹配（如"配对T检验"匹配"配对T检验"目录）
...
- 用例难度标签必须根据每条用例自身内容独立判断（前置条件复杂度、步骤数、是否跨模块联动等），不得照搬参考模板中其他用例的难度标签。
- 预期结果以 UI 设计图和 PRD 需求文档为准，不得直接套用其他算法或模块的预期结果表述。若 UI 图与 PRD 描述存在冲突（如字段名称、数据结构、字数限制等），必须停止生成并询问用户以哪个为准，再继续。
...
- 当用户提示"根据 xxx.docx 的某章节生成测试用例"或指定任意 Word PRD 时，必须先将整份 Word 转换并覆盖写入 `inputs/requirements/current_prd.md`，再从 `current_prd.md` 中提取章节生成用例；不得仅使用 `--section --print` 直接读取 Word 内容后跳过落地。
- Word 转换覆盖 `current_prd.md` 时，原始 Word 文件仍保留在 `inputs/requirements/raw_docs/` 作为来源存档；输出用例元信息需同时记录原始 Word 路径和实际读取的 `inputs/requirements/current_prd.md`。
...
- [ ] 若本次输入为 Word PRD，已先转换整份文档并覆盖 `inputs/requirements/current_prd.md`
- [ ] 已从 `inputs/requirements/current_prd.md` 提取本次指定章节的需求内容
- [ ] 已检查 `inputs/ui_design/<章节名>/` 是否有实际 UI 图片
...
- [ ] 已确认每条新生成用例的 `备注` 来源符合 `generation_rules/testcase_writing_guidelines.md`
...
| **另存** | 保留现有文件，将本次用例保存为带时间戳的新文件：`<module_name>_testcases_YYYYMMDD_HHMMSS.md`（仅另存时才带时间戳） | 使用标准表头另存 |

不允许在未获得用户明确选择前自行决定处理方式，不允许静默覆盖已有文件。

带时间戳的另存文件不会被脚本默认扫描；校验或导出另存文件时，必须使用 `--source` 显式指定该文件路径。
...
6. 生成本次输出用例，按确认的处理方式和标准表头写入；每条新用例的 `备注` 必须写明来源
...
来源章节：<章节名>
输入文件：
  - inputs/requirements/raw_docs/<文件名>.docx（最后修改：YYYY-MM-DD）
  - inputs/requirements/current_prd.md（由上述 Word 转换覆盖，最后修改：YYYY-MM-DD）
  - inputs/ui_design/<章节名>/（最后修改：YYYY-MM-DD）
...
| 输入来源 | 读取位置 | 重点提取 |
|---|---|---|
| Word 文档（推荐） | 先转换覆盖 `inputs/requirements/current_prd.md`，原始文件保留在 `inputs/requirements/raw_docs/<文件名>.docx` | 从转换后的 `current_prd.md` 提取章节内容 |
| Markdown PRD | `inputs/requirements/current_prd.md` | 用户指定章节、验收标准、主流程、异常处理 |
| UI 设计图 | `inputs/ui_design/<章节名>/` | 页面元素、按钮状态、错误提示 |

### Word 文档直接输入规则

用户提供 Word 文件路径和章节名时（如"根据 tangyao_prd.docx 的报告编制章节生成用例"），按以下步骤处理，无需用户手动转换：

1. 先运行转换命令，将整份 Word 覆盖写入默认当前 PRD：

```bash
python scripts/convert_docx.py inputs/requirements/raw_docs/<文件名>.docx --overwrite
```

2. 从 `inputs/requirements/current_prd.md` 中读取用户指定章节及必要上下文，作为本次需求内容。
3. 检查 `inputs/ui_design/<章节名>/` 是否有 UI 图片，有则一并读取。
4. 若用户未指定章节，可先运行以下命令列出 Word 章节，再请用户确认章节名；确认后仍必须执行第 1 步覆盖 `current_prd.md`：

```bash
python scripts/convert_docx.py inputs/requirements/raw_docs/<文件名>.docx --list-sections
```
5. 不使用 `--section "<章节名>" --print` 作为正式生成入口；该方式只允许用于临时排查转换问题，不得替代覆盖 `current_prd.md`。
6. 元信息块的"输入文件"必须同时填写原始 Word 文件路径和转换后的 `inputs/requirements/current_prd.md`。

## 输出规则

### 备注来源规则

`备注` 来源写法以 `generation_rules/testcase_writing_guidelines.md` 的“备注、标签和关联字段”为唯一规则源；生成和追加用例都必须按该规则填写，不在本文件重复维护细节。
...
| Excel 导出文件 | `outputs/excel_exports/<site_type>/<module_name>_testcases.xlsx` |

单个需求 / 单个模块交付时，必须显式传入单个 Markdown 源文件，导出为同名 Excel：`<module_name>_testcases.xlsx`。不带 `--source` 或传入目录时，脚本会按站点分类合并多个 Markdown 文件并生成 `测试用例导出_YYYYMMDD_HHMMSS.xlsx`，仅适合临时汇总，不作为默认交付方式。
...
校验支持 `--json` 输出结构化问题明细。导出支持 `--strict` 在 WARN 时也停止导出；默认只在 ERROR 时停止。`export_testcases.py` 会按 `difficulty_level_rules.md` 将 Excel 中的 `用例标签` 校正为推断难度；如果 Markdown 源文件也需要 0 WARN，必须先根据 `validate_cases.py` 的 WARN 明细修复源文件。

## Python 环境要求

运行脚本需使用 Python 3.10 或更高版本。`validate_cases.py`、`export_testcases.py` 和 `case_utils.py` 仅使用 Python 标准库；`convert_docx.py` 需要额外安装 `python-docx`。完整跨平台环境说明见 `README.md`。
...
- 新生成用例的 `备注` 必须按 `generation_rules/testcase_writing_guidelines.md` 写明具体来源
