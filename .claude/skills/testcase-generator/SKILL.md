...
description: 根据淘宝项目的需求文档、界面设计图、业务规则和已有参考用例，生成测试用例并导出为 Excel。适用于登录、搜索、商品详情、购物车、结算下单、多页签操作等淘宝前端测试场景。
---
...
为淘宝类电商前端项目生成结构化功能测试用例。基于需求文档、界面设计图、业务说明或已有用例，生成可执行、可导出的测试用例。
...
9. `testcase_templates/modules/<level1_menu>/<level2_menu>/module_overview.md` 和 `testcases.md`（按 CPV 菜单路径匹配，作为参考，只读）
10. 用户本次提供的需求、界面设计图或补充说明

**模块参考用例读取规则：**

- `testcase_templates/modules/` 按 CPV 系统一级菜单 / 二级菜单组织，目录名必须使用小写英文 snake_case，例如 `testcase_templates/modules/public_site/site_management/testcases.md`。
- 若需求明确对应二级菜单，优先读取该二级菜单目录下的 `module_overview.md` 和 `testcases.md`。
- 若需求只对应一级菜单，或二级菜单目录不存在，则读取一级菜单目录下的 `module_overview.md` 和 `testcases.md`（如存在）。
- 若同一需求跨多个二级菜单，应读取所有相关二级菜单参考用例；参考用例只读，不得修改。
...
- [ ] 已按 CPV 一级菜单 / 二级菜单路径读取对应参考用例作为只读参考
...
- 参考对应 `testcase_templates/modules/<level1_menu>/<level2_menu>/testcases.md` 末尾的"覆盖的验收标准"表作为格式范例。
...
脚本默认读取 `outputs/origin_exports/` 下的生成文件；需要校验参考用例时，再显式传入 `--source testcase_templates/modules`，脚本会递归扫描一级菜单和二级菜单下的 `testcases.md`。
