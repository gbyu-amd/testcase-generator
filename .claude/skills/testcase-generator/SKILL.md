...
- [ ] 已确认本次生成文件的用例编号起始值（新建从 001，追加从已有最大编号递增）

### 输出文件冲突处理

生成前必须检查 `outputs/origin_exports/<module_name>_testcases.md` 是否已存在：
...
| 选项 | 说明 | 编号处理 |
|---|---|---|
| **追加** | 在现有文件末尾补充本次新场景，保留已有用例 | 读取已有最大编号，在此基础上递增 |
| **覆盖** | 清空现有文件，重新生成全量用例 | 所有类型编号从 001 开始 |
| **另存** | 保留现有文件，将本次用例保存为带时间戳的新文件：`<module_name>_testcases_YYYYMMDD.md` | 所有类型编号从 001 开始 |
...
6. 生成本次输出用例，按确认的处理方式和编号规则写入
7. 在用例表格后追加"需求覆盖率对照表"，逐条列出 PRD 验收标准 / 需求点与覆盖它的用例编号，标注未覆盖项
8. 保存到目标文件
9. 运行校验脚本：`python scripts/validate_cases.py --source outputs/origin_exports/<module_name>_testcases.md`
10. 如果校验失败，必须根据问题明细修复 Markdown 用例文件，然后重新运行校验
11. 校验通过后运行导出脚本：`python scripts/export_testcases.py --source outputs/origin_exports/<module_name>_testcases.md`
12. 回复导出文件路径、用例数量、覆盖模块和需求覆盖率（已覆盖 / 需求总数）
