...
Markdown 用例文件包含元信息块、需求问题清单、用例统计摘要、标准用例表和需求覆盖率对照表。标准用例表使用连续分组列加固定业务字段；所有已存在的分组列都必须填写，分组名称建议使用子菜单或子功能名，没有对应到更深层级时用当前最后一级非空分组名补齐后续分组列，且相同分组名应相邻展示。若 PRD / UI 未明确具体字段、范围、格式、状态、影响对象或生效条件，且现有规则或测试团队既有判定口径无法给出可执行预期，会在需求问题清单中标记；直接依赖待确认项的用例名称前加 `【待确认】`。
