...
读取图片前必须检查 `inputs/ui_design/` 下是否存在跨目录同名文件（如多个目录下都有 `config.png`、`result.png`）。存在同名文件时，不得直接读取原路径（CDN 上传按文件名存储会导致同名冲突），必须先将图片复制到临时目录并重命名为 `<目录名>_<文件名>` 格式（如 `新增工艺驾驶舱_config.png`），再读取临时目录文件。读取图片后如分析结果与当前章节明显不符，应怀疑同名冲突并排查。
...
- 不得从参考模板、其他分析方法、历史输出或经验推断当前需求的确定结论。
...
| 业务站点 | `business_site` | `outputs/origin_exports/business_site/` | `outputs/excel_exports/business_site/` | 产品与基础数据、年度计划与任务、方案编制、监控项目、数据分析、一键分析、报告编制、业务站点系统配置与异步任务 |
...
| 一键分析、替换数据源、数据处理、未分析原因 | 默认随具体分析方法写入对应方法文件；仅独立一键分析专项需求写入 `business_site/one_click_analysis_testcases.md` |
...
已有用例对应的 PRD 被替换为新版本时，必须对照新 PRD 逐条检查已有用例，不得跳过比对直接覆盖或忽略差异。

### 检查流程

1. **提取并比对**：用 `extract_docx.py` 分别提取新旧 PRD 对应章节，逐条比对需求点差异。
2. **分类差异**：将差异分为三类——**需求变更**（新旧 PRD 对同一需求点的描述不一致）、**需求遗漏**（旧 PRD 未涉及、新 PRD 新增的需求点）、**无变化**（新旧 PRD 一致）。
3. **逐条处理已有用例**：按差异分类联动修改主表用例。

### 用例标签处理规则

| 差异类型 | 用例处理 | 用例标签 |
|---|---|---|
| 新旧 PRD 不一致，导致已有用例的步骤、预期结果、前置条件或优先级需要修改 | 修改用例内容 | 追加 `需求变更` |
| 新 PRD 新增了旧 PRD 未覆盖的需求点，需要补充新用例 | 新增用例 | 新增用例追加 `需求遗漏` |
| 新旧 PRD 一致，用例无需修改 | 不动 | 不加标记 |

- 标签追加方式：在原有难度等级后用分号 `；` 分隔追加，如 `困难` → `困难；需求变更`。
- 同一条用例若同时涉及变更和遗漏（修改后还补充了新场景），两个标记都加：`困难；需求变更；需求遗漏`。
- **仅因 PRD 版本变更检查才追加变更类标记**，常规新生成用例不得携带 `需求变更` 或 `需求遗漏`。

### 需求问题清单联动

- 新旧 PRD 差异中存在无法直接判定结论的歧义，按「需求审查清单」和「需求问题清单」规则写入需求问题清单，状态为 `待确认`。
- 差异已明确（如新 PRD 删除了旧 PRD 的某需求点），不写入需求问题清单，直接按新 PRD 处理用例并追加对应标记。

### 备注更新

- 因 PRD 变更而修改的用例，`备注` 来源段保持原值（如 `来源：prd`），不替换为新 PRD 文件名；如需说明变更来源，在 `备注` 末尾追加 `；PRD 变更检查`。
- 因需求遗漏新增的用例，`备注` 按正常来源规则填写。
